...
int get_device_count_on_bus(int bus)
{
	...
	while (next != NULL) {
		if (next->is_bridge)
			count += get_device_count_on_bridge(&bridge->bridge_info);
		else
			count++;
...
int scan_pci(void)
{
	...
	while ((dev = pci_device_next(iter)) != NULL) {
		...
		if (is_bridge(dev)) {
			...
			pci_info->bridge_info.primary_bus = prim_bus;
			pci_info->bridge_info.secondary_bus = sec_bus;
			pci_info->bridge_info.subordinate_bus = sub_bus;
